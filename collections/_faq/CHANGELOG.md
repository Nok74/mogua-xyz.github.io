...
- In Windows the start menu item is now mogua Network and the icon in Add/Remove is updated.
...
- The Windows installer is now signed by a mogua Network certificate. It may take some time to develop enough reputation to not warn multiple times during install.
...
- We now codesign the Apple .dmg installer with the mogua Network developer ID on both GitHub Actions and Azure Pipelines. We will be notarizing and distributing the Azure Pipelines version as it's built on MacOS Mojave (10.14.6) for stronger cross version support.
